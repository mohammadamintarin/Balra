...
@section('content')
    ...
    <div class="statistics flex mx-auto justify-between items-center bg-statistics w-[90%] h-25 my-6 p-8 shadow">
        <div class="flex gap-x-2 items-center">
            ...
            <span>تاکنون {{$count ?? ''}} تخفیف در فروشگاه ثبت شده است!</span>
        </div>
        <div><a class="btn btn-success" href="{{route('master.discount.create')}}">افزودن تخفیف </a></div>
    </div>
                    ...
                    @foreach($discounts as $item)
                        <tr class="item{{$item->id}}">
                            ...
                            <td class="flex items-center justify-center gap-x-3 text-gray-300">
                                <a class="group relative inline-block px-3" href="{{route('master.discount.changeToDefualt' , ['discount' => $item->id])}}">
