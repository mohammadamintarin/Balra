        ...
        <div class="container">
            <div class="bg-white shadow border border-gray-100 dark:bg-zinc-500 w-[90%] mx-auto data_more_less">
                <div class="header flex justify-between items-center border-b border-gray-200 p-5 px-10">
                    ...
                    <div>
                        <img class="rounded-full w-52" src="{{url('images/contact/thumbnail/' . $contact->image)}}" alt="{{$contact->title}}">
                    </div>
                </div>
                <form action="{{route('master.contact.update' , ['contact' => $contact->id])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="body p-4 py-9">
                        ...
                        <div class=" grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-2">
                                <label> لوکیشن</label>
                                <input type="text" name="coordinates" value="{{$contact->coordinates}}">
                            </div>
                            <div class="sm:col-span-2">
                                <label> آدرس</label>
                            ...
                            <div class="sm:col-span-1">
                                <label>تلگرام</label>
                                <input type="text" name="telegram" value="{{$contact->telegram}}">
                            </div>
                            <div class="sm:col-span-1">
                                <label>واتس اپ</label>
                            ...
                            <div class="sm:col-span-1">
                                <label>بله</label>
                                <input type="text" name="bale" value="{{$contact->bale}}">
                            </div>
                            <div class="sm:col-span-1">
                                <label>ایتا</label>
                                <input type="text" name="eita" value="{{$contact->eita}}">
                            </div>
                        </div>
                        <div class="w-full">
                            <label>درباره ما</label>
                            <textarea  name="about" rows="4" cols="50" style="height: 200px">{{$contact->about}}</textarea>
                        </div>
                    </div>
