    ...
    <section class="mb-52">
        <div class="container">
            <div class="bg-white shadow border border-gray-100 dark:bg-zinc-500 w-[90%] mx-auto data_more_less">
                ...
                <form action="{{route('master.blog.update' , ['blog' => $blog->id])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="body p-4 py-9">
                        ...
                        <div class=" grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="en" value="{{$blog->en}}" required>
                            </div>
                            <div class="sm:col-span-2">
                                <label>والد</label>
                                <select name="parent_id">
                                    ...
                                    @foreach($blogs as $key => $value)
                                        <option value="{{$key}}" {{$blog->parent == $key ? "selected" : "   "}}>{{$value}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="sm:col-span-2">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="text" name="canonical" value="{{$blog->canonical}}">
                            </div>
                        </div>
                        <div class="w-full">
                            <label class="block">محتوا</label>
                            <textarea id="editor" name="contents">{!! $blog->content !!}</textarea>
                        </div>
                    </div>
