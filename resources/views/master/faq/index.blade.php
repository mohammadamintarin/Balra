...
@section('content')
    ...
    <div class="statistics flex mx-auto justify-between items-center bg-statistics w-[90%] h-25 my-6 p-8 shadow">
        <div class="flex gap-x-2 items-center">
            ...
            <span>تاکنون {{$count ?? ''}} سوال در فروشگاه ثبت شده است!</span>
        </div>
        <div><a class="btn btn-success" href="{{route('master.faq.create')}}">افزودن سوال جدید</a></div>
    </div>
                ...
                <table class="w-full mx-auto table-auto text-center border-none">
                    ...
                    @foreach($faqs as $item)
                        <tr class="item{{$item->id}}">
                            <td>{{++$counter}}</td>
                            <td>{{\Illuminate\Support\Str::limit($item->question , 50)}}</td>
                            <td class="flex items-center justify-center gap-x-3 text-gray-300">
                                <a class="group relative inline-block px-3" href="{{route('master.faq.edit' , ['faq' => $item->id])}}">
                                ...
                                <a class="group relative inline-block px-3" onClick="deleteItem({{$item->id}} ,'{{$item->name}}' , '{{csrf_token()}}' , 'type')" href="javascript:void(0)">
