        ...
        <div class="container">
            <div class="statistics shadow-lg flex mx-auto justify-between items-center bg-statistics w-[90%] h-25 my-6 p-8 border border-gray-100">
                <div class="flex gap-x-2 items-center">
                    ...
                    <span>تاکنون {{$count ?? ''}} بنر در فروشگاه ثبت شده است!</span>
                </div>
                <div class="flex gap-x-1">
                    <a href="/master/banner/create" class="text-white bg-teal-500 px-4 py-2 rounded-sm border-2 transition-all border-teal-500 hover:bg-transparent hover:text-teal-500">
                        افزودن بنر
                    </a>
                </div>
            </div>
                        ...
                        <table class="w-full mx-auto table-auto text-center border-none">
                            ...
                            @foreach($banners as $banner)
                                <tr class="item{{$banner->id}}">
                                    <td>{{++$counter}}</td>
                                    <td><img src="{{url('images/banner/' . $banner->image)}}" alt="{{$banner->name}}"  style="width: 50px;margin: 0 auto" ></td>
                                    <td>{{$banner->name}}</td>
                                    <td class="flex items-center justify-center gap-x-3 text-gray-300" >
                                        <a class="group relative inline-block px-3" href="{{route('master.banner.edit' , ['banner' => $banner->id])}}">
                                        ...
                                        <a class="group relative inline-block px-3" onClick="deleteItem({{$banner->id}} ,'{{$banner->name}}' , '{{csrf_token()}}')" href="javascript:void(0)">
