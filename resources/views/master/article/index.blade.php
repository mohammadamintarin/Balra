...
@section('content')
    ...
    <div class="statistics flex mx-auto justify-between items-center bg-statistics  h-25 my-6 p-8 shadow">
        <div class="flex gap-x-2 items-center">
            ...
            <span>تاکنون {{$count ?? ''}} مقاله در فروشگاه ثبت شده است!</span>
        </div>
        <div><a class="btn btn-success" href="{{route('master.article.create')}}">افزودن مقاله جدید</a></div>
    </div>
                    ...
                    @foreach($articles as $item)
                        <tr class="item{{$item->id}}">
                            ...
                            <td class="flex items-center justify-center gap-x-3 text-gray-300">
                                <a class="group relative inline-block  px-3" href="/article/{{$item->slug}}">
                                ...
                                <a class="group relative inline-block px-3" href="{{route('master.article.edit' , ['article' => $item->id])}}">
