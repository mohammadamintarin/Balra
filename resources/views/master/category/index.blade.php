...
@section('content')
    ...
    <div class="statistics flex mx-auto justify-between items-center bg-statistics w-[90%] h-25 my-6 p-8 shadow">
        <div class="flex gap-x-2 items-center">
            ...
            <span>تاکنون {{$count ?? ''}} دسته‌بندی در فروشگاه ثبت شده است!</span>
        </div>
        <div><a class="btn btn-success" href="{{route('master.category.create')}}">افزودن دسته‌بندی جدید</a></div>
    </div>
                ...
                <table class="w-full mx-auto table-auto text-center border-none">
                    ...
                    @foreach($categories as $item)
                        <tr class="item{{$item->id}}">
                            <td>{{++$counter}}</td>
                            <td>{{$item->name}}</td>
                            <td>@if($item->parent_id == 0) -- @else {{$item->parent->name}} @endif</td>
                            <td></td>
                            <td>{{$item->viewed}}</td>
                            <td class="flex items-center justify-center gap-x-3 text-gray-300">
                                <a class="group relative inline-block  px-3" href="/category/{{$item->slug}}">
                                ...
                                <a class="group relative inline-block px-3" href="{{route('master.category.edit' , ['category' => $item->id])}}">
...
@endsection
@include('master.category.delete')
